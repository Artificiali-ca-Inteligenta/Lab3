...
def reachableNodes(startPoint, pointList, shapeList, edgeList, maxDist):
    nodes = []
    for p in pointList:
            ...
            if kante.p1 == p or kante.p2 == p or kante.p1 == startPoint or kante.p2 == startPoint:
                ...
                for shape in shapeList:
                    if shape.contains(line):
                        isIntersecting = True
        if isIntersecting is False and startPoint.calculateDistance(p)<maxDist:
            nodes.append(p)
    return nodes
...
def main():
    ...
    for nrRep in range(2):  
        ...
        dictVecini = {}
        dictVecini[startPoint]=reachableNodes(startPoint,points,shapeList, edgeList, maxDist)
        dictVecini[endPoint]=reachableNodes(endPoint,points, shapeList, edgeList, maxDist)
        for point in points:
            dictVecini[point]=reachableNodes(point,points, shapeList, edgeList, maxDist)
        ...
        for node in points:
            for vecin in dictVecini[node]:
                line = shapely.geometry.LineString([[node.x, node.y], [vecin.x, vecin.y]])
                ax.plot(*np.array(line).T, color='blue', linewidth=1, solid_capstyle='round')
        for vecin in dictVecini[startPoint]:
            line = shapely.geometry.LineString([[startPoint.x, startPoint.y], [vecin.x, vecin.y]])
            ax.plot(*np.array(line).T, color='blue', linewidth=1, solid_capstyle='round')
        for vecin in dictVecini[endPoint]:
            line = shapely.geometry.LineString([[endPoint.x, endPoint.y], [vecin.x, vecin.y]])
            ax.plot(*np.array(line).T, color='blue', linewidth=1, solid_capstyle='round')
